...
import { OrderAddEdit } from '../OrderAddEdit/OrderAddEdit';
import { DeleteOrder } from '../DeleteOrder/DeleteOrder';

export const InventoryList = ({state}: {state: IState}) => {
    const {isPending, error, data} = useGetOrders();
        ...
        <div className={style.main}>
                ...
                <div className="" ref={contentToPrint}>
                    ...
                    <div className="">
                        <div className={style.header}>
                            ...
                            <div className="">Заказчик</div>
                            <div className="">Выдал</div>
                            <div className="">Итого</div>
                        </div>
                        {data.map((item: IOrders, index: number) => (
                            <div className={style.item} key={`${item.id}_${index}`}>
                                <div className="">
                                    <OrderAddEdit
                                        btnText={
                                        ...
                                        value={{
                                            ...
                                            person_in: item.person_in,
                                            person_out: item.person_out,
                                            sum: item.sum,
                                            id: item.id
                                        }}
                                    />
            ...
            <div className={style.buttomBtns}>
                <div style={{display: 'flex', gap: '30px'}}>
                    <OrderAddEdit btnText={'Добавить ивентаризацию'} titlePopup={'Добавление ивентаризации'}/>
                    <Link to="/"><button>Сотрудники</button></Link>
                </div>
                <button onClick={() => {
